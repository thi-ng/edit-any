                    ...
                    (str "<h2>Attribs</h2>"
                         ...
                         (->> page
                              (map (fn [{:syms [?att ?atitle ?val ?vtitle]}]
                                     (str "<tr><td>" (resource-link ?att ?atitle) "</td><td>" (resource-link ?val ?vtitle) "</td></tr>")))
                              (apply str))
                         "</table>"))
                  (when (or (seq others) (seq others2))
                    (str "<h2>Other resources</h2>"
                         "<table>"
                         (->> others
                              (map (fn [{:syms [?other ?otitle ?val ?vtitle]}]
                                     (str "<tr><td>" (resource-link ?other ?otitle) "</td><td>" (or ?title id) "</td><td>" (resource-link ?val ?vtitle) "</td></tr>")))
                              (apply str))
                         (->> others2
                              (map (fn [{:syms [?other ?otitle ?pred ?ptitle]}]
                                     (str "<tr><td>" (resource-link ?other ?otitle) "</td><td>" (resource-link ?pred ?ptitle) "</td><td>" (or ?title id) "</td></tr>")))
                              (apply str))
                         "</table>")))))
  :post! (fn [ctx]
           (dosync
            (let [{:keys [body title attribs]} (get-in ctx [:request :params])
                  triples (->> (map (fn [[p o]] [id (name p) o]) attribs)
                               (concat [[id "dcterms:content" body]
                                        [id "rdfs:label" title]
                                        [id "dcterms:modified" (System/currentTimeMillis)]]))]
              (prn :triples triples)
